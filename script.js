...
(() => {
  ...
  function ensureSparkles() {
    ...
    const zones = [[2,24],[76,98],[24,36],[64,76],[36,64],[42,58],[8,92]];
    for (let i = 0; i < 70; i++) {
  ...
  function setupMusic() {
    ...
    // v64 best-effort music autoplay: browsers may block WebAudio until a user gesture.
    button.textContent = baseLabel;
    ...
    async function attemptAutoplay() {
      ...
      try {
        ...
        if (ctx && ctx.state === 'running') {
          document.body.classList.remove('music-autoplay-blocked');
          return;
        }
      } catch (e) {}
      if (!playing) {
        document.body.classList.add('music-autoplay-blocked');
        button.textContent = isHebrew ? 'מוזיקה — לחצו' : 'Music — click';
      }
    }
    function unlockAfterGesture() {
      if (!playing) start();
      document.removeEventListener('pointerdown', unlockAfterGesture);
      document.removeEventListener('keydown', unlockAfterGesture);
    }
    window.setTimeout(attemptAutoplay, 650);
    document.addEventListener('pointerdown', unlockAfterGesture, { once: true });
    document.addEventListener('keydown', unlockAfterGesture, { once: true });
